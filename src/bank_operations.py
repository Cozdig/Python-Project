...
def re_sort(list_transactions, search):
    ...
    operations = [
        operations
        for operations in list_transactions
        if pattern.search(operations.get("description", ""))
    ]
    return operations

def count_category_dict(operations, category_op):
    """Эта функция принимает список словарей с данными о банковских операциях и список категорий операций, а возвращает словарь."""
